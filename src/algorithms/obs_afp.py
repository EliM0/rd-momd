...
class ObsAveragePolicy(rl_agent.AbstractAgent):
  ...
  def __init__(self,
    ...
    def avg_network_policy(param, state):
      action_values = self.avg_network.apply(param, state)
  ...
  def _get_update_fn(self):
    ...
    def update(param_avg, opt_state_avg, info_states, action_probs, distributions):
      states = jax.numpy.hstack((info_states, distributions))
      loss_val, grad_val = self._loss_and_grad(param_avg, states,
                                               action_probs)
  ...
  def _act(self, state, legal_actions) -> Tuple[int, np.ndarray]:
  ...
  def step(self,
    ...
    if is_evaluation:
      ...
      distribution = time_step.observations['distribution'][self._player_id]

      state = info_state + distribution

      action, probs = self._act(state, legal_actions)
  ...
  def _loss_avg(self, param_avg, states, action_probs):
    avg_logit = self.avg_network.apply(param_avg, states)
    loss_value = -jnp.sum(
  ...
  def learn(self) -> Optional[float]:
    ...
    self._params_avg_network, self._opt_state, loss_val_avg = self._jit_update(
        self._params_avg_network, self._opt_state, info_states, action_probs, distributions)
