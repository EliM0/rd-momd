...
class TrajectoryMunchausenDQN(rl_agent.AbstractAgent):
  ...
  def reset(self, force_timer=True):
    if (force_timer and self._trajectory_save_timer > 0 and len(self._trajectory_sample) > self._trajectory_sample_length
        and not self._trajectory_sample[-1].is_final_step):
      assert len(self._trajectory_sample) >= self._trajectory_sample_length
      self._trajectory_replay_buffer.add(self._trajectory_sample[-self._trajectory_sample_length:])
...
class SoftMaxTrajectoryMunchausenDQN(rl_agent.AbstractAgent):
  """Wraps a Munchausen DQN agent to use soft-max action selection."""

  def __init__(self, agent: TrajectoryMunchausenDQN, tau: Optional[float] = None):
    self._agent = agent
    self._tau = tau
...
class TrajectoryDeepOnlineMirrorDescent(object):
  ...
  def __init__(self,
    ...
    for agent in agents:
      assert isinstance(agent, TrajectoryMunchausenDQN)
  ...
  def get_softmax_policy(self,
    ...
    return rl_agent_policy.JointRLAgentPolicy(
        self._game, {
            idx: SoftMaxTrajectoryMunchausenDQN(agent, tau=tau)
