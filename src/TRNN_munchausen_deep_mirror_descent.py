...
TrajectoryStep = collections.namedtuple(
    "TrajectoryStep",
    "info_state action legal_one_hots reward is_final_step")
...
class TRNNMunchausenDQN(rl_agent.AbstractAgent):
...
# Trajectory change 4: modify buffer, adding trajectory 
  ...
  def _epsilon_greedy(self, info_state, legal_actions, epsilon):
    ...
    # Change 5: output of apply 
    # q_values, _ = self.hk_network_apply(self._params_q_network, info_state)
  ...
  def _softmax(self, info_state, legal_actions,
    ...
    # print("before info_state softmax: ", info_state.shape)
    # info state is passed in as list of size 67
    info_state = np.reshape(info_state, [1, -1])

    # info_state is array of shape (1, 67)
    # print("info_state softmax: ", info_state.shape)
    # Change 6: output of apply 
    # q_values, _ = self.hk_network_apply(self._params_q_network, info_state)
    # print("_ ", _)
    q_values = self.hk_network_apply(self._params_q_network, info_state)[0]
...
class SoftMaxTRNNMunchausenDQN(rl_agent.AbstractAgent):
  """Wraps a Munchausen DQN agent to use soft-max action selection."""

  def __init__(self, agent: TRNNMunchausenDQN, tau: Optional[float] = None):
    self._agent = agent
    self._tau = tau
...
class TRNNDeepOnlineMirrorDescent(object):
  ...
  def __init__(self,
    ...
    for agent in agents:
      assert isinstance(agent, TRNNMunchausenDQN)
  ...
  def get_softmax_policy(self,
    ...
    return rl_agent_policy.JointRLAgentPolicy(
        self._game, {
            idx: SoftMaxTRNNMunchausenDQN(agent, tau=tau)
