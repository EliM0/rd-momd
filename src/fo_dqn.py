...
Transition = collections.namedtuple(
    "Transition",
    "info_state action reward next_info_state is_final_step legal_actions_mask distribution next_distribution")
...
class DQN(rl_agent.AbstractAgent):
  ...
  def _get_action_probs(self, states, legal_actions, is_evaluation=False):
    """Returns a selected action and the probabilities of legal actions."""
    epsilon = self._get_epsilon(is_evaluation)
    return self._epsilon_greedy(states, legal_actions, epsilon)
  ...
  def step(self, time_step, is_evaluation=False, add_transition_record=True):
    ...
    if (not time_step.last()) and (time_step.is_simultaneous_move() or
      ...
      action, probs = self._get_action_probs(
          state, legal_actions, is_evaluation=is_evaluation)
    else:
      action = None
      probs = []
  ...
  def _loss(self, param, param_target, info_states, actions, rewards,
            next_info_states, are_final_steps, legal_actions_mask, distributions, next_distributions):

    states = jax.numpy.hstack((info_states, distributions))
    next_states = jax.numpy.hstack((next_info_states, next_distributions))

    q_values = self.hk_network.apply(param, states)
    target_q_values = self.hk_network.apply(param_target, next_states)
  ...
  def learn(self):
    ...
    self.params_q_network, self._opt_state, loss_val = self._jit_update(
        ...
        legal_actions_mask, distributions, next_distributions)
